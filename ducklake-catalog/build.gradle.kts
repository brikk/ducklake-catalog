...
mavenPublishing {
    ...
    coordinates("dev.brikk.ducklake", "ducklake-catalog", version.toString())
    pom {
        name.set("DuckLake Catalog")
        description.set(
            "JVM (Kotlin/jOOQ) client for the DuckLake catalog metadata schema — the shared " +
                "catalog layer consumed by the Trino and Doris DuckLake connectors.",
        )
...
dependencies {
    // platform (not enforcedPlatform): this module is PUBLISHED, and an enforced platform
    // leaks as a forced constraint to consumers (Gradle blocks it in generateMetadataFile).
    // A plain platform aligns the Kotlin stdlib without forcing downstream resolution.

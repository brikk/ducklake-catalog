...
class TestJdbcDucklakeCatalogOnMySqlSmoke {
    ...
    @Test
    fun partialRewriteIgnoresInlinedDeleteTablesInAnotherDatabase() {
        val catalog = catalog!!
        val server = server!!
        ...
        DriverManager.getConnection(server.getJdbcUrl(), "root", server.getPassword()).use { connection ->
            connection.createStatement().use { statement ->
                statement.execute("CREATE DATABASE unrelated_partial")
                ...
                statement.execute("GRANT SELECT ON unrelated_partial.* TO '${server.getUser()}'@'%'")
            }
        }
